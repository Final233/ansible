...
_run() {
    ...
    if [ $# -ge 2 ]; then
        file_list=$(find /home/tdgw/AUTOF* -name ${file_name})
        time=$2
        ...
        # file_list=$(find /home/tdgw/AUTOF* -name ${file_name} | xargs grep 'pbu"' | tr -d '":,' | awk '{print $1,$NF}')
        # file_time=$(find /home/tdgw/AUTOF* -name ${file_name} | xargs grep 'knockDoorInAdvanceMs' | awk '{print $1,$NF}' | tr -d ,)
